...
def make_api_request(query):
    ...
    json_data = {
        "query": query, "documents": False, "id_usuario": "Test", "id_cliente": "Test"
    }
...
for idx, row in tqdm(searchable.iterrows(), total=searchable.shape[0]):
    ...
    country = "Peru"
    statement = f"""
        INSERT INTO {database}.[elastic].[Prod_Resultados_Elastic_{country}] 
        (Fecha, Termino_Busqueda, Termino_Busqueda_Dicc, Numero_Refax, Aplicacion, Pais) 
        VALUES (%s, %s, %s, %s, %s, %s)
    """
    data = list(map(lambda x: (x["fecha_hora"], x["busqueda_real"], x["busqueda"], x["Numero_Refax"], x["Aplicacion"], country), insert_p_data))
    cursor = conn.cursor()
    ...
    i += 1
    if (i > 0 and i % 1000 == 0):
        statement = """INSERT INTO """+database_70+""".metricas.buscador_elastic (busqueda, busqueda_real, fecha_hora, resultados) VALUES (%s, %s, %s, %s)"""
